...
复用现有关系标明每次导入的店铺与渠道；[CSV 导出 PR #27](https://github.com/crz0614/marketplace-payment-loop/pull/27)安全导出当前账户视图；[状态归一 PR #28](https://github.com/crz0614/marketplace-payment-loop/pull/28)保留平台原始值，并由 PostgreSQL 生成待付款、待处理、已发货、已完成、已取消、退款或其他标准状态；[全量汇总 PR #29](https://github.com/crz0614/marketplace-payment-loop/pull/29)增加受限的账户级服务端汇总，不受 200 条账本上限影响，并按标准状态与币种统计去重订单、商品数量和导入金额，明确不把金额称为收入。CI 共通过 32 项测试；线上多币种检查把同一 Amazon 订单的两个 SKU 去重为 1 单、3 件、USD 20.34，另行汇总 USD 5.00 退款与 CNY 99.00 淘宝待处理订单；清理后测试用户、会话、店铺、导入和订单均为零。七个平台的直接
